...
/**
 * 图片加载引擎，用于EasyPhotos库
 * Glide4.x的加载图片引擎实现,单例模式
 * Glide4.x的缓存机制更加智能，已经达到无需配置的境界。如果使用Glide3.x，需要考虑缓存机制。
 * Created by ZuoHailong on 2019/5/12.
 */
public class ImageEngineForEasyPhotos implements ImageEngine {

    //单例
    private static ImageEngineForEasyPhotos instance = null;

    //单例模式，私有构造方法
    private ImageEngineForEasyPhotos() {
    }

    //获取单例
    public static ImageEngineForEasyPhotos getInstance() {
        if (null == instance) {
            synchronized (ImageEngineForEasyPhotos.class) {
                if (null == instance) {
                    instance = new ImageEngineForEasyPhotos();
                }
            }
        }
        return instance;
